                ...
                foreach (var (serverName, apiKey) in arkSAKeys)
                {
                    ...
                    try
                    {
                        ...
                        results.Add(new
                        {
                            ...
                            MaxPlayers = data["maxplayers"],
                            VoteUrl = $"{data["url"]}/vote",
                            ConnectUrl = $"https://zlg.gg/discord"
                        });
                    }
                    catch (HttpRequestException ex)
                    {
                ...
                foreach (var (serverName, apiKey) in empyrionKeys)
                {
                    ...
                    try
                    {
                        ...
                        using (var jsonDocument = JsonDocument.Parse(responseData))
                        {
                            ...
                            // Helper function to safely read JSON elements
                            ...
                            results.Add(new
                            {
                                ...
                                Version = GetStringValue(data.GetProperty("version"))?.Replace("[^\\d.]", ""),  // Clean non-digit characters
